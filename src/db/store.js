  ...
  async upsertScore(openid, score) {
    await this.pool.execute(`
      ...
      ON DUPLICATE KEY UPDATE
        ...
        week_round = IF(VALUES(week_key) <> week_key, VALUES(week_round), IF(VALUES(week_score) > week_score, VALUES(week_round), week_round)),
        week_best_wave_gain = IF(VALUES(week_key) <> week_key, VALUES(week_best_wave_gain), IF(VALUES(week_score) > week_score, VALUES(week_best_wave_gain), week_best_wave_gain)),
        week_revived = IF(VALUES(week_key) <> week_key, VALUES(week_revived), IF(VALUES(week_score) > week_score, VALUES(week_revived), week_revived)),
        week_updated_at = IF(VALUES(week_key) <> week_key, CURRENT_TIMESTAMP, IF(VALUES(week_score) > week_score, CURRENT_TIMESTAMP, week_updated_at)),
        week_score = IF(VALUES(week_key) <> week_key, VALUES(week_score), GREATEST(week_score, VALUES(week_score))),
        day_key = VALUES(day_key),
        day_round = IF(VALUES(day_key) <> day_key, VALUES(day_round), IF(VALUES(day_score) > day_score, VALUES(day_round), day_round)),
        day_best_wave_gain = IF(VALUES(day_key) <> day_key, VALUES(day_best_wave_gain), IF(VALUES(day_score) > day_score, VALUES(day_best_wave_gain), day_best_wave_gain)),
        day_revived = IF(VALUES(day_key) <> day_key, VALUES(day_revived), IF(VALUES(day_score) > day_score, VALUES(day_revived), day_revived)),
        day_updated_at = IF(VALUES(day_key) <> day_key, CURRENT_TIMESTAMP, IF(VALUES(day_score) > day_score, CURRENT_TIMESTAMP, day_updated_at)),
        day_score = IF(VALUES(day_key) <> day_key, VALUES(day_score), GREATEST(day_score, VALUES(day_score)))
    `, [
      openid,
      score.nickname,
      score.avatar,
